    ...
    <div>
      <S.UserList>
        ...
        <S.Filters>
          ...
          <CheckBox value="FI" label="Finland" onChange={(country) => changeCountry(country)} />
        </S.Filters>
        </S.FilterContainer>
          ...
          {users.map((user, index) => {
            if (users.length === index + 1) {
              return <div ref={lastUser} key={user.login.uuid}>
                <User user={user} index={index} hoveredUserId={hoveredUserId} setHoveredUserId={setHoveredUserId} changeUserFavorite={changeUserFavorite} handleMouseEnter={handleMouseEnter} handleMouseLeave={handleMouseLeave} isUserFavorite={isUserFavorite} />
              </div>
            }
            else {
